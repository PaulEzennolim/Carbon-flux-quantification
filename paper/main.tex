...
TEMPO is one of a fast-growing family of time-series foundation models.
Chronos~\cite{TODO_chronos} tokenises series and reuses a language-model
backbone; TimesFM~\cite{TODO_timesfm} is a decoder-only forecaster pretrained
on a large temporal corpus; Moirai~\cite{TODO_moirai} targets universal,
any-frequency forecasting; and Lag-Llama~\cite{TODO_lagllama} is an
open probabilistic foundation model for time series. These share the
pretrain-once, forecast-anywhere premise we test here; we evaluate TEMPO as a
representative instance and leave a multi-model comparison to future work.
% NOTE (item 6): the four citations above are PLACEHOLDERS (\cite{TODO_*}) and
% are NOT yet in references.bib. Paul to confirm he has read each and add a
% verified entry himself. Candidate papers:
%   TODO_chronos  -- Ansari et al., "Chronos: Learning the Language of Time
%                    Series," TMLR 2024 (arXiv:2403.07815).
%   TODO_timesfm  -- Das et al., "A decoder-only foundation model for
%                    time-series forecasting," ICML 2024 (arXiv:2310.10688).
%   TODO_moirai   -- Woo et al., "Unified Training of Universal Time Series
%                    Forecasting Transformers" (Moirai), ICML 2024
%                    (arXiv:2402.02592).
%   TODO_lagllama -- Rasul et al., "Lag-Llama: Towards Foundation Models for
%                    Probabilistic Time Series Forecasting," 2023
%                    (arXiv:2310.08278).
% Keep total references <= 20 (currently 11 real entries + these 4 = 15 max).
...
pretrained prior already aligns with its regular signal.
...
concrete: the forest exhibits a pronounced, regular day--night NEE oscillation,
whereas the wetland's cycle is comparatively flat and irregular---precisely the
distinction between the in-distribution periodic signal a generic time-series
prior forecasts well and the suppressed, out-of-distribution signal it does not.

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{figures/diurnal_patterns_by_ecosystem.pdf}
  \caption{Mean diurnal NEE by ecosystem. The forest shows a strong, regular
  photosynthetic day--night cycle; the wetland's flux is suppressed and
  irregular, consistent with the alignment mechanism for the inverted penalty.}
  \label{fig:diurnal}
\end{figure}
...
\begin{figure}[t]
  ...
  $5{,}000$-sample protocol and so differ slightly from Table~\ref{tab:penalty}.}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{figures/model_comparison_by_horizon.png}
...
\begin{table}[t]
  ...
  Source: \texttt{pairwise\_comparisons.csv}.}
  \label{tab:sig}
  \small
  \begin{tabular}{@{}llccc@{}}
    \toprule
...
\begin{table}[t]
  ...
  Source: \texttt{computational\_costs.json},
  \texttt{efficiency\_comparison.csv}.}
  \label{tab:cost}
  \small
  \begin{tabular}{@{}lccc@{}}
    \toprule
