...
bog) and one forest (SE-Htm). The benchmark is our instrument, not our object:
...
\begin{itemize}
  \item \textbf{First empirical characterisation of cross-ecosystem transfer in a
        time-series foundation model.} We present, to our knowledge, the first
        systematic investigation of how a pretrained time-series foundation model
        transfers across ecosystem boundaries for carbon-flux forecasting.
  \item \textbf{Evidence of an inverted cross-ecosystem transfer penalty.} Contrary
        to the expectation of classical transfer learning, we observe that the
        pretrained foundation model achieves higher forecasting accuracy on a
        cross-ecosystem forest than on a held-out wetland, while conventional
        machine-learning baselines exhibit the expected degradation.
  \item \textbf{Towards understanding transfer behaviour in foundation models.} We
        analyse the observed inversion and provide evidence that alignment between
        the target ecosystem dynamics and the model's pretraining distribution may
        be a stronger determinant of transfer performance than ecosystem similarity
        alone. We also explicitly discuss the principal limitations of the current
        study and identify directions for future validation.
\end{itemize}
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.85\textwidth]{figures/nee_timeseries_all_sites.pdf}
...
cycle. Full method and per-site values are in
\texttt{results/analysis/signal\_regularity.csv} and summarised in
Table~\ref{tab:regularity}.
...
\begin{table}[t]
  ...
  (dash otherwise). Source: \texttt{results/analysis/signal\_regularity.csv}.}
