...
on a large temporal corpus; One Fits All~\cite{zhou2023one} adapts a frozen
pretrained language-model backbone to time series; and
Lag-Llama~\cite{rasul2024lag} is an open probabilistic foundation model for
time series. These share the pretrain-once, forecast-anywhere premise we test
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{figures/nee_timeseries_all_sites.pdf}
  ...
  held-out test sites (UK-AMo wetland, SE-Htm forest).}
  \label{fig:neetimeseries}
\end{figure}

\paragraph{Covariates.}
% Source: scripts/tempo_data_prep.py (meteorological_vars [10], modis_bands [7],
% temporal_features [2] = 19 covariates/timestep)
The multivariate baselines receive $19$ covariates per timestep, in three
groups: ten meteorological and surface-energy variables (incoming shortwave and
longwave radiation, vapour-pressure deficit, air temperature, air pressure,
precipitation, wind speed, and the ground, latent and sensible heat fluxes),
...
periodic structure the pretrained model represents well---so the foundation
model tracks it with low, predictable error throughout the day. On the wetland
site the hourly error is flatter and noisier, lacking the strong diurnal
signature, consistent with a suppressed and irregular flux that is harder to
anticipate from a generic temporal prior. The contrast in error structure thus
echoes the alignment argument: the forest's regularity is what the model
exploits, not merely the magnitude of its flux.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.49\columnwidth]{figures/error_by_hour_SE-Htm.pdf}\hfill
  \includegraphics[width=0.49\columnwidth]{figures/error_by_hour_UK-AMo.pdf}
  \Description{Two side-by-side panels of forecast error against hour of day.
  The forest panel (left) shows a regular daily oscillation in error that
  follows the day-night cycle; the wetland panel (right) shows a flatter, noisier
  error with no clear diurnal shape.}
  \caption{Forecast error by hour of day for the forest site (SE-Htm, left) and
  the wetland site (UK-AMo, right). The forest shows a regular diurnal error
