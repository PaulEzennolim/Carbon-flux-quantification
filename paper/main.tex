...
wetland (UK-AMo) and one forest (SE-Htm). The conventional models exhibit
the expected penalty, degrading by 29--61\% in $R^2$ when transferring from
...
wetland site---a $-21.6\%$ ``penalty'', i.e.\ an improvement when crossing
...
\begin{itemize}
  \item \textbf{The inverted transfer penalty.} We document that fine-tuned
        ...
        than the same-ecosystem wetland site ($R^2=0.599$), a $-21.6\%$
        penalty, while three conventional baselines degrade by $29$--$61\%$
...
\[
  \mathrm{penalty} = \frac{R^2_{\mathrm{wetland}}-R^2_{\mathrm{forest}}}
                          {R^2_{\mathrm{wetland}}},
\]
so a positive value is the classically expected degradation and a negative
value is an inversion. Table~\ref{tab:penalty} reports these values from the
dedicated transfer protocol, and Figure~\ref{fig:transfermatrix} shows the
full transfer matrix.

% Source: results/transfer_learning/TRANSFER_LEARNING_SUMMARY.txt (section H4)
%         results/transfer_learning/transfer_matrix.csv (100%-coverage config)
% NOTE: the transfer protocol uses matched 5,000-sample training subsamples,
% so baseline R^2 here differ slightly from the full-test benchmark in Table 2.
% TEMPO R^2 are identical across both (0.599 wetland / 0.728 forest).
\begin{table}[t]
  \centering
  ...
  baselines degrade by $29$--$61\%$; fine-tuned TEMPO improves by $21.6\%$.
  Source: \texttt{TRANSFER\_LEARNING\_SUMMARY.txt} (H4),
  \texttt{transfer\_matrix.csv}.}
  \label{tab:penalty}
  \small
  \begin{tabular}{@{}lccc@{}}
    \toprule
    Model & UK-AMo $R^2$ & SE-Htm $R^2$ & Penalty \\
          & (wetland) & (forest) &  \\
    \midrule
    Random Forest    & 0.559 & 0.332 & $+0.227$\ ($+40.6\%$) \\
    XGBoost          & 0.532 & 0.376 & $+0.157$\ ($+29.4\%$) \\
    LSTM             & 0.468 & 0.184 & $+0.285$\ ($+60.8\%$) \\
    \textbf{TEMPO Fine-Tuned} & 0.599 & 0.728 & $\mathbf{-0.129}$\ ($\mathbf{-21.6\%}$) \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[t]
  ...
  \caption{Cross-ecosystem transfer matrix. The baselines lose substantial
  skill moving from the wetland to the forest site, whereas TEMPO gains skill.}
  \label{fig:transfermatrix}
\end{figure}

Every baseline pays the expected penalty---$+40.6\%$ (Random Forest),
$+29.4\%$ (XGBoost) and $+60.8\%$ (LSTM)---losing skill exactly where theory
predicts. Fine-tuned TEMPO does the opposite, recording a $-21.6\%$ penalty:
...
a $+93.7\%$ relative $R^2$ advantage over the best baseline, versus a modest
$+7.1\%$ on the wetland site. The differences are statistically decisive: on
...
Sweeping training-data coverage ($20$--$100\%$) and leave-one-site-out
removals over the three baselines, $33$ of $60$ model--configuration--site
combinations ($55\%$) exhibit negative transfer, where adding source data
\emph{harms} target performance relative to the full-data reference. The
...
classical cross-ecosystem transfer penalty---degrading by $29$--$61\%$ in
