...
\begin{itemize}
  \item \textbf{The inverted transfer penalty.} We show that fine-tuned
        ...
        across this identical boundary.
  \item \textbf{Baseline-versus-foundation contrast.} Across coverage and 
        leave-one-site-out setups, $33$ of $60$ baseline configurations 
        ($55\%$) exhibit negative transfer, isolating a failure mode that 
        the foundation model avoids.
  \item \textbf{Mechanism and limitations.} We attribute the inversion to
        alignment between the forest's regular diurnal photosynthetic cycle
...
\begin{itemize}
  ...
  \item \textbf{LSTM}: A two-layer recurrent network utilizing $\sim$221k parameters.
\end{itemize}
...
\begin{table}[t]
  \centering
  \caption{\bfseries\boldmath Primary 96-hour forecasting benchmark on the two held-out test
...
\begin{table}[t]
  \centering
  \caption{\bfseries\boldmath Cross-ecosystem transfer penalty (wetland $\rightarrow$ forest),
...
\begin{figure}[t]
  ...
  \Description{Two vertically stacked panels of forecast error against hour of day.
  The wetland panel (top) shows a flatter, noisier error with no clear diurnal shape; 
  the forest panel (bottom) shows a regular daily oscillation in error that follows the day-night cycle.}
  \caption{Forecast error by hour of day for the wetland site (UK-AMo, top) and
  the forest site (SE-Htm, bottom). The forest shows a regular diurnal error
...
\begin{figure}[t]
  ...
  \caption{\bfseries\boldmath Transfer matrix for the three baselines (Random Forest, XGBoost,
...
\begin{figure}[t]
  ...
  \caption{\bfseries\boldmath $R^2$ versus forecast horizon ($1$--$96$\,h). On the forest site
...
\begin{table}[t]
  \centering
  \caption{\bfseries\boldmath Fine-tuned TEMPO vs.\ each baseline: Cohen's $d$, paired $t$-test
...
inverts this behavior, forecasting the forest target ($R^2=0.728$) with higher 
...
outweigh a nominal ecosystem match. Future work will equalize model inputs 
